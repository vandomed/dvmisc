...
#' the variable names.
#' @param kable Logical value for whether to prepare for
...
clean_glm <- function(
  ...
  if (! is.logical(kable)) {
    stop("The input 'kable' should be TRUE if you plan to print via 'kable' and FALSE otherwise.")
  }
    ...
    for (ii in 1: length(variable.classes)) {
      if (variable.classes[ii] == "factor") {
        ...
        row.labels <- c(row.labels, variable_labels[ii],
                        paste(ifelse(kable, "\\ \\ ", "  "), factor.levels[[variable.names[ii]]], sep = ""))
      } else {
        row.labels <- c(row.labels, variable_labels[ii])
      }
    }
